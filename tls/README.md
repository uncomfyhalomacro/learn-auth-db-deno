The certificates in this dir expire on Sept, 13th, 2123. Determined using
command `openssl x509 -enddate -nout -in <Path To Cert>`
...
```shell
openssl req -new -nodes -newkey rsa:2048 -keyout localhost.key -out localhost.csr -subj "/C=US/ST=YourState/L=YourCity/O=Example-Certificates/CN=127.0.0.1.local/CN=localhost.local"
